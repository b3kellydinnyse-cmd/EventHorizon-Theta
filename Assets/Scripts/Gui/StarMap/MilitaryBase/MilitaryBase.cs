...
namespace Gui.StarMap
{
    public class MilitaryBase : MonoBehaviour
    {
        ...
        // Check master switch for premium currency
        private bool IsStarsAllowed => _database.EconomySettings == null || _database.EconomySettings.EnablePremiumCurrency;
        ...
        // Update player resources indicators
        private void UpdateResources()
        {
            _creditsText.text = _playerResources.Money.ToString();
            _starsText.text = _playerResources.Stars.ToString();

            // Toggle star panel based on master switch
            _starsPanel.SetActive(IsStarsAllowed);
        }

        private int Level => Mathf.Max(5, _motherShip.CurrentStar.Level / 2);

        // Calculate level-up price in stars or fallback to credits
        private Price GetLevelUpPrice(IShip ship)
        {
            var size = 1 + Mathf.Max(0, (int)ship.Model.SizeClass);
            var price = 1 + ship.Experience.Level * size / 25;

            return IsStarsAllowed ? Price.Premium(price) : Price.Common(price * 1000);
        }
    }
}