...
namespace Services.Localization
{
    internal class Localization
    {
        ...
        private void AddPluralForm(System.Text.StringBuilder builder, string format, Parameters parameters)
        {
            ...
            if (!int.TryParse(items[0], out var paramId) || paramId <= 0 || paramId > parameters.Count)
            {
                AddInvalidParameter(builder, paramId);
                return;
            }

            // Parse as long to prevent OverflowException
            long.TryParse(parameters[paramId - 1].ToString(), out var hugeCount);

            // Use 100 for out-of-bounds values to satisfy plural grammar rules in all languages
            int count = (hugeCount > int.MaxValue || hugeCount < int.MinValue) ? 100 : (int)System.Math.Abs(hugeCount);
        ...
        private void AddEnumeration(System.Text.StringBuilder builder, string format, Parameters parameters)
        {
            ...
            long.TryParse(parameters[0].ToString(), out var hugeCount);
            var index = 1 + (int)(System.Math.Abs(hugeCount) % (items.Length - 1));
