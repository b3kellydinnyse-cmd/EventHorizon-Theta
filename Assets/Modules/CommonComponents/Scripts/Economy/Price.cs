...
namespace Economy
{
    public struct Price
    {
        // Global flag to allow star usage
        public static bool AllowStars = true;

        // Conversion rate from stars to credits when stars are disabled
        public static int StarsToCreditsRate = 500;
        ...
        public static Price Premium(Money amount)
        {
            if (AllowStars)
            {
                // Return original star price
                return new Price(amount, Currency.Stars);
            }
            else
            {
                // Convert stars to credits if premium currency is disabled
                return new Price(amount * StarsToCreditsRate, Currency.Credits);
            }
        }
