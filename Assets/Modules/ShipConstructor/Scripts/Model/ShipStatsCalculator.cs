...
namespace Constructor.Model
{
    ...
    public class ShipStatsCalculator : IShipStats
    {
        ...
        public float ArmorRepairRate
        {
            get
            {
                float baseHullRegen = 0f;

                if (_ship.Features.Regeneration)
                {
                    // If a value is specified (not 0), we take it.
                    // If 0, then for older ships we return the classic 1% (0.01f).
                    baseHullRegen = _ship.Features.RegenerationAmount != 0 ? _ship.Features.RegenerationAmount : 0.01f;
                }

                var regeneration = baseHullRegen + BaseStats.RegenerationRate;

                var repairFromModules = EquipmentStats.ArmorRepairRate * Bonuses.ArmorPointsMultiplier.Value * BaseStats.ArmorMultiplier.Value;
                var repairFromHull = ArmorPoints * regeneration;

                // Now, if regeneration is negative, the ship will literally take damage over time if there are no repair modules
                return repairFromModules + repairFromHull;
            }
        }
        ...
        public StatMultiplier DroneSpeedMultiplier => EquipmentStats.DroneSpeedMultiplier + BaseStats.DroneSpeedMultiplier;
        public StatMultiplier DroneRangeMultiplier => EquipmentStats.DroneRangeMultiplier + BaseStats.DroneRangeMultiplier;
        ...
        private float DroneReconstructionSpeed => (ShipSettings.BaseDroneReconstructionSpeed +
         EquipmentStats.DroneReconstructionSpeed + BaseStats.DronesBuiltPerSecondMultiplier) * BaseStats.DroneBuildSpeedMultiplier.Value;
