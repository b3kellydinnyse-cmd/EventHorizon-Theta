...
namespace GameDatabase.Storage
{
    public class DatabaseContent : IContentLoader
    {
        ...
        public void LoadJson(string name, string content)
        {
            ...
            else if (type == ItemType.EconomySettings)
            {
                if (EconomySettings == null)
                {
                    var data = _serializer.FromJson<EconomySettingsSerializable>(content);
                    data.FileName = name;
                    EconomySettings = data;
                    return;
                }

				if (!_allowDuplicates)
                    throw new DatabaseException("Duplicate EconomySettings file found - " + name);
            }
		...
		public EconomySettingsSerializable EconomySettings { get; private set; }
